...
class DashboardAPIHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 1b. API: GET /api/portfolio_us_stocks
        if self.path == '/api/portfolio_us_stocks':
            portfolio_file = os.path.join(dir_path, 'portfolio_us_stocks.json')
            if not os.path.exists(portfolio_file):
                self.send_error(404, "portfolio_us_stocks.json not found")
                return
            ...
            try:
                ...
                self.send_error(500, f"Error reading US stocks portfolio: {str(e)}")
            return

        # 1c. API: GET /api/portfolio_us_dcs
        if self.path == '/api/portfolio_us_dcs':
            portfolio_file = os.path.join(dir_path, 'portfolio_us_dcs.json')
            if not os.path.exists(portfolio_file):
                self.send_error(404, "portfolio_us_dcs.json not found")
                return
        ...
        # 1i. API: GET /api/portfolio_strategy10
        if self.path == '/api/portfolio_strategy10':
            portfolio_file = os.path.join(dir_path, 'portfolio_strategy10.json')
            if not os.path.exists(portfolio_file):
                self.send_error(404, "portfolio_strategy10.json not found")
                return
    ...
    def do_POST(self):
        ...
        # API: POST /api/backtest-strategy10
        if self.path == '/api/backtest-strategy10':
            try:
