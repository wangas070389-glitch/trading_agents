...
def run_strategy12_backtest_for_api():
    p = PARAMS
    data = load_real_data()
    res = run_backtest(data, p)
    m = perf_metrics(res["nav"], p["rf_mxn"])
    ...
    cash_vals = [float(x) for x in res["bondia"].values]
    bench_vals = [float(x) for x in res["bh_qqq"].values]
    ...
    return {
        ...
        "metrics": {
            ...
            "cash_return": float((res["bondia"].iloc[-1] / initial_nav - 1.0) * 100),
            "benchmark_return": float((res["bh_qqq"].iloc[-1] / initial_nav - 1.0) * 100),
            "benchmark_cagr": float(m_bh["cagr"] * 100),
