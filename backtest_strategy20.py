"""
Strategy 20: Hurst Exponent & Fractional Brownian Motion Systematic QQQ / TQQQ / SQQQ Allocation
==============================================================================================
This strategy uses Fractional Brownian Motion principles to dynamically select the QQQ
exposure regime. It estimates the rolling Hurst Exponent (H_t) of QQQ log prices:
  - H_t > 0.52: Persistent trending market. We allocate to TQQQ (3x Long) or SQQQ (3x Short)
    depending on the direction of a dual-SMA filter (50-day vs. 120-day SMA).
  - H_t <= 0.52: Chop/mean-reverting market. We exit to Cash (Bondia cash sweep at 6.53%)
...
def calculate_hurst_exponent_rolling(log_prices, window_size=100, max_lag=20):
    ...
    ln(std(p_{t+tau} - p_t)) = H * ln(tau) + C
    """
    n = len(log_prices)
...
def run_simulation(data, initial_nav=200000.0):
    ...
    # Calculate rolling Hurst Exponent on log prices
    log_closes = np.log(data["qqq"].values)
    hurst_values = calculate_hurst_exponent_rolling(log_closes, window_size=100, max_lag=20)
    
    # Smooth Hurst slightly to filter short-term noise
    hurst_series = pd.Series(hurst_values)
    hurst_smoothed = hurst_series.rolling(window=5, min_periods=1).mean().values
    ...
    # Trending threshold: H > 0.52. Chop threshold: H <= 0.52.
    # Hysteresis band: exit trending only if H <= 0.49
    for t in range(1, n_days):
        h_val = hurst_smoothed[t-1]
        ...
        if np.isnan(fast_sma) or np.isnan(slow_sma):
            ...
            # Evaluate Hurst regime
            if current_asset in (2, 3):
                # We currently hold a trending asset (TQQQ/SQQQ)
                # Exit if Hurst drops below 0.38 (decay warning) or trend flips
                if h_val > 0.38:
                    if current_asset == 2:
                ...
                # Enter trending asset if Hurst breaches 0.42
                if h_val > 0.42:
                    target_asset = 2 if is_bull else 3
                else:
                    target_asset = 0
    ...
    df_out["hurst_raw"] = hurst_values
    df_out["hurst_smoothed"] = hurst_smoothed
...
def main():
    ...
    print("=" * 80)
    print("STRATEGY 20: HURST EXPONENT SYSTEMATIC BACKTEST")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("BACKTEST RESULTS: S20 HURST EXPONENT SYSTEMATIC")
    print("=" * 80)
    print(f"Backtest Period : {df_out.index[0].strftime('%Y-%m-%d')} to {df_out.index[-1].strftime('%Y-%m-%d')} ({years:.2f} Years)")
    print(f"Final NAV (S20) : ${final_nav:,.2f} MXN (Benchmark QQQ Buy&Hold: ${bench_final:,.2f} MXN)")
    ...
    # Export Report
    report_md = f"""# Strategy 20 Backtest Report (Hurst Exponent Dynamic Allocation)
...
"""
    ...
    with open(os.path.join(dir_path, "strategy20_backtest_report.md"), "w", encoding="utf-8") as f:
        f.write(report_md)
        
    df_out.to_csv(os.path.join(dir_path, "strategy20_backtest_nav.csv"))
    print(f"Saved backtest NAV curve and logs successfully to: strategy20_backtest_nav.csv")
    print(f"Saved backtest markdown report successfully to: strategy20_backtest_report.md")
