...
def main():
    ...
    # Parse S23
    s23_market_val = s23_cash = s23_invested = 0.0
    if port_strategy23:
        s23_total_cap = 200000.0
        s23_cash = port_strategy23.get("cash_balance", 200000.0)
        s23_invested = sum(h["shares"] * h.get("buy_price", 0.0) for h in port_strategy23.get("holdings", []))
        s23_market_val = s23_cash + sum(h["shares"] * valuation_price(h) for h in port_strategy23.get("holdings", []))
        s23_profit = s23_market_val - s23_total_cap
        s23_roi = (s23_profit / s23_total_cap) * 100.0
        s23_alloc = (s23_invested / s23_market_val) * 100.0 if s23_market_val > 0 else 0.0
        s23_sign = "+" if s23_profit >= 0 else ""
        report.append(f"| **S23: Calculus S&R & RSI Systematic** | ${s23_market_val:,.2f} | ${s23_cash:,.2f} | ${s23_invested:,.2f} | {s23_alloc:.1f}% | {s23_sign}${s23_profit:,.2f} | {s23_sign}{s23_roi:.2f}% | 2026-07-15 | MXN |")
    else:
        report.append("| **S23: Calculus S&R & RSI Systematic** | *Not Initialized* | - | - | - | - | - | - | MXN |")

    # Parse S24
    s24_market_val = s24_cash = s24_invested = 0.0
    if port_strategy24:
        s24_total_cap = 200000.0
        s24_cash = port_strategy24.get("cash_balance", 200000.0)
        s24_invested = sum(h["shares"] * h.get("buy_price", 0.0) for h in port_strategy24.get("holdings", []))
        s24_market_val = s24_cash + sum(h["shares"] * valuation_price(h) for h in port_strategy24.get("holdings", []))
        s24_profit = s24_market_val - s24_total_cap
        s24_roi = (s24_profit / s24_total_cap) * 100.0
        s24_alloc = (s24_invested / s24_market_val) * 100.0 if s24_market_val > 0 else 0.0
        s24_sign = "+" if s24_profit >= 0 else ""
