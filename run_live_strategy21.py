"""
LIVE EXECUTION: STRATEGY 21 SHANNON ENTROPY SYSTEMATIC QQQ / TQQQ / SQQQ
========================================================================
Runs daily to evaluate the Shannon Information Entropy on QQQ daily returns
...
# Local settings
PORTFOLIO_FILE = "portfolio_strategy21.json"
TRANSACTIONS_FILE = "transactions_strategy21.md"
REPORT_FILE = "strategy21_report_live.md"
...
def main():
    ...
    print("=" * 80)
    print(f"LIVE EXECUTION: STRATEGY 21 SHANNON ENTROPY SYSTEMATIC ({today_str})")
    print("=" * 80)
    ...
    # 5. Fetch QQQ history and run Shannon Entropy
    print("\nLoading trailing QQQ daily return series...")
    try:
        qqq_history = yf.download("QQQ", period="150d", progress=False)
        ...
        qqq_returns = qqq_closes.pct_change().dropna()
        
        # Calculate rolling Entropy
        entropy_values = calculate_shannon_entropy_rolling(qqq_returns.values, window_size=60, num_bins=10)
        
        # Smooth entropy (last 5 values)
        entropy_smoothed = float(pd.Series(entropy_values).iloc[-5:].mean())
        
        # Calculate SMAs
        fast_sma = float(qqq_closes.iloc[-50:].mean())
        slow_sma = float(qqq_closes.iloc[-120:].mean())
        
        print(f"Shannon Entropy Estimates:")
        print(f"  Smoothed Normalized Entropy (H_t): {entropy_smoothed:.4f}")
        print(f"  QQQ Fast SMA (50d): ${fast_sma:.2f} USD")
        print(f"  QQQ Slow SMA (120d): ${slow_sma:.2f} USD")
    except Exception as e:
        print(f"Error executing Shannon engine: {e}. Defaulting to Cash.")
        entropy_smoothed = 0.95
        fast_sma = 100.0
        slow_sma = 100.0
    ...
    # Thresholds: entry_thresh = 0.85, exit_thresh = 0.88
    if current_asset_name in ("TQQQ", "SQQQ"):
        # Currently holding trending asset: exit if Entropy > 0.88 or trend flips
        if entropy_smoothed < 0.88:
        ...
        # Currently holding CASH: enter trend if Entropy <= 0.85
        if entropy_smoothed <= 0.85:
            target_asset_name = "TQQQ" if is_bull else "SQQQ"
        else:
            target_asset_name = "CASH"
    ...
    if target_asset_name != current_asset_name:
        ...
        if held_ticker:
            ...
            if not args.dry_run:
                ...
                log_transaction(dir_path, today_str, held_ticker, "SELL", held_shares, px_mxn, f"Entropy switch to {target_asset_name}", fee=fee_mxn)
        ...
        if target_asset_name != "CASH":
            ...
            if shares_to_buy > 0.0001:
                ...
                print(rebalance_logs[-1])
                if not args.dry_run:
                    log_transaction(dir_path, today_str, target_asset_name, "BUY", shares_to_buy, px_mxn, f"Entropy systematic entry", fee=fee_mxn)
    ...
    report_md = f"""# Strategy 21: Shannon Entropy Live Execution Report
...
"""
    ...
    report_md += f"* **Rolling Normalized Shannon Entropy ($H_t^{{norm}}$):** {entropy_smoothed:.4f}\n"
    report_md += f"* **Information Regime Mode:** {'ORDERED (Trend Capture Active)' if entropy_smoothed <= 0.85 else 'DISORDERED/NOISY (Cash Safety Active)'}\n"
    ...
    print("=" * 80)
    print("LIVE PAPER TRADING SUMMARY - STRATEGY 21")
    print("=" * 80)
    print(f"  NAV:             ${portfolio_value:,.2f} MXN")
    print(f"  Holding Target:  {target_asset_name}")
    print(f"  Shannon Entropy: {entropy_smoothed:.4f}")
