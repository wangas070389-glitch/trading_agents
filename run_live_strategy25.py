"""
Strategy 25 LIVE Runner: Golden MACD Systematic (BMV Value Stocks)
==================================================================
Runs daily to rebalance a 5-ticker BMV value portfolio using Golden parameters.
"""
import os
import sys
import json
...
PORTFOLIO_FILE = "portfolio_strategy25.json"
TRANSACTIONS_FILE = "transactions_strategy25.md"
REPORT_FILE = "strategy25_report_live.md"
...
def log_transaction(dir_path, date_str, ticker, action, shares, price, note, fee=0.0):
    ...
    if not os.path.exists(t_path):
        with open(t_path, "w", encoding="utf-8") as f:
            f.write("# Strategy 25 Transaction Ledger (Golden MACD BMV)\n\n"
...
def main():
    ...
    print("=" * 80)
    print(f"LIVE EXECUTION: STRATEGY 25 GOLDEN MACD BMV ({today_str})")
    print("=" * 80)
    ...
    # Download recent data (last 200 days to calculate 55 EMA)
    print("Downloading stock data...")
    try:
        data = yf.download(tickers, period="200d", group_by="ticker", progress=False)
        fx_now = yf.Ticker("MXN=X").history(period="1d")
        if data.empty or fx_now.empty:
    ...
    for ticker in tickers:
        ...
        if holding is None:
            ...
            if close_t > ma_t and crossover_bull:
                target_alloc = portfolio["total_capital"] * 0.18 # 18% alloc limit per stock
                if cash >= target_alloc:
                    ...
                    log_transaction(dir_path, today_str, ticker, "BUY", shares, close_t, "Golden MACD entry", fee)
                    print(f"[{ticker}] BUY {shares:.4f} shares @ ${close_t:.2f} MXN")
        else:
            # Evaluate exit
            ...
            if sell:
                ...
                print(f"[{ticker}] SELL {shares:.4f} shares @ ${close_t:.2f} MXN due to: {note}")
                holding = None
    ...
    with open(os.path.join(dir_path, REPORT_FILE), "w", encoding="utf-8") as f:
        f.write(f"# Strategy 25 Live Status Report\n\n")
        ...
        if new_holdings:
            ...
            f.write(f"No active stock positions.\n")
            
    print(f"Strategy 25 Live Update Completed. Portfolio NAV: ${total_val:,.2f} MXN")
