...
def main():
    ...
    for tf, lb in grid:
        try:
            print(f"Running grid parameter: Timeframe={tf}, Lookback={lb}d...")
            res = run_backtest(tf, lb, universe)
            results.append((tf, lb, res))
        except Exception as e:
            print(f"Error testing {tf} + {lb}d: {e}")
            
    print("\n" + "=" * 80)
    print("LOOKBACK DAYS GRID SEARCH RESULTS FOR S11")
    print("=" * 80)
    print(f"{'Timeframe':<10} | {'Lookback':<8} | {'Final NAV':<16} | {'Total Return':<14} | {'Sharpe':<8} | {'Max DD':<10} | {'Trades':<8}")
    print("-" * 80)
    for tf, lb, res in results:
        print(f"{tf:<10} | {lb:<8} | ${res['final_nav']:,.2f} MXN | {res['total_return']:+12.2f}% | {res['sharpe']:8.2f} | {res['max_dd']:9.2f}% | {res['trade_count']:<8}")
    print("=" * 80)
