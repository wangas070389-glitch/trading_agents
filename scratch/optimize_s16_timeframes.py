...
def run_backtest_for_timeframe(tf, universe, download_period):
    ...
    for base, assets in universe.items():
        if tf == "4h":
            ...
            interval = tf
            if tf == "1d":
                # For 1d, 60d period is small, download 1y
    ...
    # 2. Run simulation over the common window of the last 60 days
    INITIAL_NAV = 200000.0
    cash = INITIAL_NAV
    active_pos = None
    nav_history = []
    
    # We restrict backtest to the last 60 days of dates available for all
    ...
    for date_str in backtest_dates:
        # Decode daily HMM using the last 30 periods of each timeframe
        regimes = {}
        scores = {}
        
        for base in universe.keys():
            df = intraday[base]
            # Get historical bars up to yesterday
            ...
            # Use strictly the last 30 periods for HMM training as requested
            train_closes = sub_df["Close_base"].iloc[-30:]
...
def main():
    ...
    print("\n" + "=" * 80)
    print("TIMEFRAME OPTIMIZATION RESULTS FOR S16 (M-HMM 30 PERIOD LOOKBACK)")
    print("=" * 80)
